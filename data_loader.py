...
class Recipe1MDataset(data.Dataset):
    ...
    def __init__(self, data_dir, aux_data_dir, split, maxseqlen, maxnuminstrs, maxnumlabels, maxnumims,
                 transform=None, max_num_samples=-1, use_lmdb=False, suff='', weight = False):
        ...
        if weight:
            self.dataset = pickle.load(open(os.path.join(aux_data_dir, suff + 'recipe1m_'+split+'_weight.pkl'), 'rb'))
        else:
